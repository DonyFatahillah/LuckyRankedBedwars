  ...
  try {
    ...
    for (const member of players) {
      ...
      if (player.elo < eloQueue.minElo || player.elo > eloQueue.maxElo) {
        await member.voice.disconnect().catch(() => {});
        await member.send(`❌ You must be between ${eloQueue.minElo}-${eloQueue.maxElo} ELO for ${eloQueue.type} queue.`).catch(() => {});
        continue;
      }
    ...
    const expectedCount = eloQueue.type === '3v3' ? 6 : eloQueue.type === '4v4' ? 8 : 2;
    if (validated.length < expectedCount) {
      console.log(`[ELO Queue] Not enough eligible players: ${validated.length}/${expectedCount}`);
      return;
    }

    console.log(`[ELO Queue] Starting ${eloQueue.type} match with ${validated.length} players.`);
