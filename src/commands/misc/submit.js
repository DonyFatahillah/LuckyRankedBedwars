  ...
  async execute(interaction) {
    ...
    if (screenshot.size > 25 * 1024 * 1024) {
      try {
        ...
        console.warn('[Submit] editReply failed for file size error:', err);
      }
    }
    ...
    for (const [gameId, matchData] of entries) {
      ...
      if (matchLogs[gameId]?.submitted) {
        try {
          ...
          console.warn('[Submit] editReply failed for duplicate submission:', err);
        }
      }
      ...
      if (!staffChannel) {
        try {
          ...
          console.warn('[Submit] editReply failed for missing staff channel:', err);
        }
      }
      ...
      try {
        ...
        if (err.code === 10008) {
          console.warn('[Submit] Could not edit reply: message deleted or missing.');
        } else {
          console.error('[Submit] Unexpected error editing reply:', err);
        }
      }
    ...
    try {
      ...
      if (err.code === 10008) {
        console.warn('[Submit] Fallback editReply failed: message deleted or missing.');
      } else {
        console.error('[Submit] Fallback editReply error:', err);
      }
    }
  },
