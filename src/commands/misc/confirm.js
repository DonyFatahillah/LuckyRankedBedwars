...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      for (const playerId of allPlayerIds) {
        ...
        const isTopKiller = member.user.username.toLowerCase() === topKillerUsername;
        const isWinBreaker = member.user.username.toLowerCase() === winBedbreakerUsername;
        const isLoseBreaker = loseBedbreakerUsername && member.user.username.toLowerCase() === loseBedbreakerUsername;
        ...
        await player.addRecentGame(gameId); // ✅ NEW: Recently Played Tracking
        await player.save();
  ...
  autocomplete: async (interaction) => {
    ...
    const choices = members.filter(Boolean).map(m => ({
      name: m.displayName,
