  ...
  async execute(interaction) {
    ...
    for (const member of members.filter(Boolean)) {
      ...
      results.push({
        ...
        oldElo: logEntry.players?.[member.id]?.oldElo ?? player.elo,
        newElo: logEntry.players?.[member.id]?.newElo ?? player.elo,
    ...
    await editLogEmbed(interaction.client, guild.id, gameId, STAFF_CHANNEL_ID, 'confirmed', updateData);
    await editLogEmbed(interaction.client, guild.id, gameId, MATCH_LOGS_ID, 'confirmed', updateData);
    await editLogEmbed(interaction.client, guild.id, gameId, VERIFY_MATCH_CHANNEL_ID, 'confirmed', updateData);

    return interaction.editReply({
      content: `✅ Match #${gameId} confirmation updated with ELO summary.`,
    });
  },
